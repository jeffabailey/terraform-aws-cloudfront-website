...
run "off_by_default_leaves_the_function_unchanged" {
  command = plan
  assert {
    condition     = !strcontains(aws_cloudfront_function.redirect_function[0].code, "lastIndexOf")
...
run "on_renders_a_301_for_a_directory_url" {
  ...
  assert {
    condition     = strcontains(aws_cloudfront_function.redirect_function[0].code, "uri.lastIndexOf(\".\") <= uri.lastIndexOf(\"/\")")
    error_message = "expected the directory test"
  }
  assert {
    condition     = strcontains(aws_cloudfront_function.redirect_function[0].code, "uri.indexOf(\"/.well-known/\") !== 0")
    error_message = "well-known paths must be excluded"
  }
  assert {
    condition     = strcontains(aws_cloudfront_function.redirect_function[0].code, "statusCode: 301")
